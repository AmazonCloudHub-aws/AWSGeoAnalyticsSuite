...
def upload_to_s3(processed_data, bucket_name, object_key):
    ...
    # Convert ObsPy Stream object to MiniSEED format (example)
# Store processed data in a variable
processed_stream = processed_data["data"]

# Write the processed data to a MiniSEED file
mseed_data = processed_stream.write("temp.mseed", format="mseed", byteorder='<')


    # Upload data to S3 bucket
    s3_client.upload_file("temp.mseed", bucket_name, object_key)

    print(f"data uploaded'{bucket_name}' with object key '{object_key}'")
